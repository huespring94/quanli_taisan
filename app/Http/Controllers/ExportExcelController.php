...
class ExportExcelController extends Controller
{
    ...
    public function downloadStatistic (Request $request)
    {
        ...
        return Excel::create ('ThongKe', function ($excel) use ($data) {
                $excel->sheet ($data['year'].'-'.(isset($data['faculty_id']) ? $data['faculty_id'] : $data['room_id']), function ($sheet) use ($data) {
                    $sheet->setHeight (4, 20);
                    $sheet->setAutoSize (false);
                    ...
                    if (!isset ($data['room_id'])) {
                        ...
                        $sheet->row (6, array('Khoa ', $this->facRoomService->getRoomById($data['room_id'])->name));
